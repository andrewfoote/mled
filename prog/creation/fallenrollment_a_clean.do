...
clear all
set more off
global filesdir "/home/research/masslayoff/education/raw/enrollment_a"
...
***********************************************************************************************************************************************************
*Lines below adapt Matt Naven's code

/*

local bases "tot res"
local weights "ef_\`base' prop_\`base'_state_enrollment"
local ban_states "ca tx wa fl mi ne az nh ok"
local races "min maj white black hisp asian indian"

cd $filesdir
use "fall_enrollment.dta", clear
keep unitid year efalevel line section lstudy eftotlt eftotlm eftotlw ///
	efaiant efaianm efaianw efasiat efasiam efasiaw efbkaat efbkaam efbkaaw ///
	efhispt efhispm efhispw efnhpit efnhpim efnhpiw efwhitt efwhitm efwhitw ///
	ef2mort ef2morm ef2morw efunknt efunknm efunknw efnralt efnralm efnralw ///
	  year ///
	dvefait dvefaim dvefaiw dvefapt dvefapm dvefapw dvefbkt dvefbkm dvefbkw ///
	dvefhst dvefhsm dvefhsw dvefwht dvefwhm dvefwhw efrace19 efrace05 efrace06 ///
	efrace20 efrace07 efrace08 efrace18 efrace03 efrace04 efrace21 efrace09 ///
	efrace10 efrace22 efrace11 efrace12 ///
	  efrace01 ///
	efrace02 efrace13 efrace14 efrace15 efrace16 efrace17 efrace23 efrace24 ///
	

/*merge m:1 unitid year using data/institutional_characteristics_clean.dta
*	, keepusing()
...
**** Include Unknown with Whites
/*
egen temp_ef_white_men = rowtotal(ef_white_men ef_unknown_men), missing
drop ef_white_men
rename temp_ef_white_men ef_white_men
egen temp_ef_white_wom = rowtotal(ef_white_wom ef_unknown_wom), missing
drop ef_white_wom
rename temp_ef_white_wom ef_white_wom
egen temp_ef_white_tot = rowtotal(ef_white_tot ef_unknown_tot), missing
drop ef_white_tot
rename temp_ef_white_tot ef_white_tot
*/
...
compress
save $datdir/clean_fallenra.dta, replace
*/

**** Undergraduates
use $datdir/clean_fallenra.dta, clear
...
/*merge 1:1 unitid year using data/institutional_characteristics_clean.dta
*	, keepusing(stabbr instnm iclevel control sector satvr25 satmt25 satvr75 satmt75 ///
*	actcm25 actcm75 admssn applcn)
...
sort unitid year
compress
save $datdir/fallenra_total_clean.dta, replace
...
sort unitid year
compress
save $datdir/fallenra_freshman_clean.dta, replace
