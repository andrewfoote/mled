...
eststo clear;
foreach sec in  47 {;
foreach sensi in always nosand1 nosand2 in1996{;
foreach outcome in $mainoutcomes  { ;
	sum ln_`outcome'_`sec' ;
	eststo clear ;
	local x = 1 ;
		
	eststo a`x': qui reg ln_`outcome'_`sensi'_`sec' l1_log_czlayoff  l2_log_czlayoff l3_log_czlayoff  [weight=total_pop], cluster(czone);
		sum `outcome'_`sensi'_`sec' if e(sample);
		local zz=r(mean);
	...
	eststo a`x': qui areg ln_`outcome'_`sensi'_`sec' l1_log_czlayoff  l2_log_czlayoff l3_log_czlayoff yearfe*  [weight=total_pop], absorb(czone) cluster(czone);
		sum `outcome'_`sensi'_`sec' if e(sample);
		local zz=r(mean);
		estadd scalar ysu=`zz';
	...
	eststo a`x': qui areg ln_`outcome'_`sensi'_`sec' l1_log_czlayoff  l2_log_czlayoff l3_log_czlayoff yearfe* 
	...
	`agegroups' cztre* [weight=total_pop], absorb(czone) cluster(czone);
		...
		lincom (l1_log_czlayoff +  l2_log_czlayoff +l3_log_czlayoff);
			local total_`sensi'_`outcome' = r(estimate);
			estadd	local total_beta = round(r(estimate),0.0001): a`x'  ;
			estadd	local total_se = round(r(se),0.0001): a`x'  ;
			estadd  local total_pval = string(tprob(r(df),(`total_beta'/`total_se')),"%5.4f"): a`x' ;
				local x = `x' + 1 ; 
	...
	esttab using "${tabdir}/lev/sensreg_rev`outcome'_`sensi'.tex", replace se r2 star(* .10 ** .05 *** .01) 
