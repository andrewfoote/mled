...
*DIRECTORIES
local logdir ""
local datadir "$datdir"
local mapdir "$datdir/maps"
local finishedmap "$figdir"

*DATASETS
local dataname "mapdata" 

*VARIABLES
local mapvar "$mapva"
local countyvar "fips"

*MAP OPTIONS
local mapname "$mapnam"
local clmethod "custom"
local clcuts "50 75 90"
local mapcolor "$mapcol"
...
if "`clmethod'" == "custom" { 
	...
	foreach perc in `clcuts' {
		local p`perc' = round(r(p`perc'),0.001) 
		di "`p`perc''" 
...
spmap `mapvar' using "`mapdir'/countyCOOR.dta", id(id) fcolor(`mapcolor') `cloption' saving("`finishedmap'/`mapname'.gph", replace)
	graph export "`finishedmap'/`mapname'.eps", replace
