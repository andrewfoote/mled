...
/*********************
Creating rates of layoff
************************/
gen ml_rate = total_ext/lau_LF ; 
...
foreach outcome in $mainoutcomes  { ;
	gen rate_`outcome'=  `outcome'_`sec'/total_pop; 
	local x = 1 ;
		
				eststo spec`sec'`outcome': reg rate_`outcome'_`sec' l2_demsh`v'*  L.ml_rate L2.ml_rate L3.ml_rate yearfe*  czfe*   cztre*   ,  cluster(czone);
					sum `outcome'_`sec' if e(sample);
					local zz=r(mean);
					estadd scalar ysu=`zz';
				};
				esttab * using "${tabdir}/lev/rates_main_`sec'.tex", replace se(a2) b(a2)  r2 star(* .10 ** .05 *** .01) 
				noconstant nomtitles noobs nogaps noline nonumbers compress label  prehead(" ") posthead(" ") prefoot(" ") postfoot(" ") 
				keep(L.ml_rate L2.ml_rate L3.ml_rate )
