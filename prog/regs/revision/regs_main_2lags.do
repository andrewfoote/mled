...
foreach outcome in $mainoutcomes  { ;
	...
	eststo a`x': reg ln_`outcome'_`sec' l1_log_czlayoff  l2_log_czlayoff   [weight=total_pop], cluster(czone);
		sum `outcome'_`sec' if e(sample);
		local zz=r(mean);
		estadd scalar ysu=`zz';
		
		lincom (l1_log_czlayoff +  l2_log_czlayoff );
	...
	eststo a`x': areg ln_`outcome'_`sec' l1_log_czlayoff  l2_log_czlayoff  yearfe*  [weight=total_pop], absorb(czone) cluster(czone);
		sum `outcome'_`sec' if e(sample);
		local zz=r(mean);
		estadd scalar ysu=`zz';
	...
	eststo a`x': areg ln_`outcome'_`sec' l1_log_czlayoff  l2_log_czlayoff  yearfe* 
	...
	esttab using "${tabdir}/lev/reg_main_rev`outcome'_2lags.tex", replace se r2 star(* .10 ** .05 *** .01) 
		noconstant nomtitles noobs nogaps noline nonumbers compress label  prehead(" ") posthead(" ") prefoot(" ") postfoot(" ") 
		keep(l1_log_czlayoff  l2_log_czlayoff   )
