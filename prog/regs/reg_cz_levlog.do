...
foreach outcome in $mainoutcomes  { ;
	local x = 1 ;
				...
				eststo spec`sec'`outcome': areg `outcome'_`sec' l1_log_czlayoff  l2_log_czlayoff l3_log_czlayoff yearfe*   cztre* `agegroups'    [weight=`weightvar'], absorb(czone) cluster(czone);
					sum `outcome'_`sec' if e(sample);
					local zz=r(mean);
					estadd scalar ysu=`zz';
				};
				esttab * using "${tabdir}/lev/reg_levlog_`sec'_`type'_wdemog.tex", replace se ar2 r2 star(* .10 ** .05 *** .01) 
